...
Routine implementation does not require a `gtt/CHANGE-REQUEST.md`.
...
— the ADR draft, the full text of every affected file under `gtt/context/`
(including the updated `gtt/context/stack.md`), and an executable script:
...
human execution actually writes to `gtt/adr/` and `gtt/context/stack.md` —
...
`gtt/context/stack.md` provides seven views:
...
5. dependency rules
6. map change log
7. drift signals — paths outside `gtt/` that carry architectural weight, watched by `detect-drift.py`
...
`gtt/backlog.md` is the development line: Epics, Stories, current focus, and
...
`gtt/CHANGE-REQUEST.md`, same as an architecture change. Updating a Story's
...
GTT provides one adapter per supported ADE. A project installs exactly the
...
- [ ] Establish the applicable Epic/Story from `gtt/backlog.md`, if one exists.

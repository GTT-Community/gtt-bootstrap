...
or `gtt/backlog.md`'s structure directly.

Overwrite the block below each time. This file is a desk, not an archive — the
history lives in `gtt/adr/` for architecture and in `gtt/backlog.md` itself
...
3. The agent reads this file and the relevant governed context (or
   `gtt/backlog.md`, for a development-line request), then writes a full
   proposal to `gtt/proposals/`.
...
5. On approval: an architecture/context change gets an ADR and the exact
   ...
   directly to `gtt/backlog.md` — it does not get an ADR unless it also
...
Routine Story status updates in `gtt/backlog.md` (moving a Story through
...
`gtt/context/` are written too broadly. Narrow them.

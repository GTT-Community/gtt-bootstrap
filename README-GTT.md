...
4. Checks whether `gtt/context/` still contains template placeholders.
...
Before the project is frozen, there is nothing ratified yet to protect, so the agent may write `gtt/context/` directly during this one-time bootstrap.

Freezing is a **human act**. It validates that the context no longer contains template placeholders and creates the `gtt/.frozen` marker. That marker switches the project into the governed regime, where governed paths become protected from direct agent writes.
...
For the detailed procedures, see [gtt/INSTALLATION.md](gtt/INSTALLATION.md) and [gtt/USAGE.md](gtt/USAGE.md).
...
See [gtt/INSTALLATION.md](gtt/INSTALLATION.md#manual-installation) for the complete manual procedure.
...
one native adapter. The GTT Bootstrap source distribution contains every
adapter because it is a catalog; installing all of them into a project is
not the intended flow.

**Portable core:** `AGENTS.md`, `README-GTT.md`, `README-GTT.es.md`, `SOURCE-BRIEF.*` (if a source document existed) at the project root, plus `gtt/` itself — carrying `INDEX.md`, `CHANGE-REQUEST.md`, `GTT-COMPLETION.md`, `backlog.md`, `INSTALLATION.md`/`.es.md`, `USAGE.md`/`.es.md`, `context/`, `adr/`, `proposals/`, `docs/`, `scripts/`.
...
>
> GTT requires selecting the ADE that is executing this bootstrap.
>
> Detected:
> - Claude Code
> - Kiro
>
> Please confirm which ADE is currently executing the GTT bootstrap.
...
- `AGENTS.md`, `README-GTT.md`, `README-GTT.es.md`, and `SOURCE-BRIEF.*` (when one exists) MUST remain at the project root — nothing else GTT owns does.
- `INDEX.md`, `CHANGE-REQUEST.md`, `GTT-COMPLETION.md`, and `backlog.md` MUST be generated directly under `gtt/` — never written to the project root and moved afterward.
- The GTT bootstrap README MUST be installed as `gtt/README.md`.
- GTT-owned directories (`adr/`, `context/`, `docs/`, `proposals/`, `scripts/`) MUST remain under `gtt/`.
...
**GTT keeps governed development artifacts under `gtt/`. Only ADE
discovery/integration files and human-facing GTT entry points remain at
the project root.**

The project root belongs to the user's project; `gtt/` belongs to GTT
governance. Before this principle, GTT's own governance files (`INDEX.md`,
`CHANGE-REQUEST.md`, `GTT-COMPLETION.md`, `backlog.md`) sat directly at the
...
The test: *if I am a developer using GTT, does the project root look like
my project, while `gtt/` clearly contains the GTT governance machinery?*
...
- GTT's human-facing entry points (`README-GTT.md`, `README-GTT.es.md`, `SOURCE-BRIEF.*`).
- The user's own project files.

This is a structural change, not a cleanup step: the bootstrap process
generates these artifacts directly under `gtt/` — it never writes them to
...
| **`gtt/CHANGE-REQUEST.md`** | The front door for a requested change | Whenever a governed decision or the development line needs to change |
...
lives under `gtt/` with the rest of what it governs, but it is still the
single entry point you use constantly; it never gets harder to reach just
because it moved.

`gtt/context/stack.md` is the file you will read most often — the one-screen map of what the system is — but it is an output, not a file you should normally edit by hand. Approved changes reach it through `gtt/CHANGE-REQUEST.md`, never by silently editing the governed map.

---

## The map

`gtt/context/stack.md` answers **“what is this system?”** without opening the code.
...
| 7 | Drift signals | Which paths outside `gtt/` carry architectural weight, and what do they guard? |
...
```text
gtt/CHANGE-REQUEST.md  ->  gtt/proposals/  ->  you review + run one script  ->  gtt/adr/ + gtt/context/stack.md
      you state intent          agent drafts           the Human Promotion Boundary       governed/protected
      always writable           agent writable
```

Fill in the request block in `gtt/CHANGE-REQUEST.md` with what needs to change, why, trigger, scope, impact, risk, and priority.
...
You approve the proposal. The agent then stages a **promotion package** in `gtt/proposals/`: the ADR draft, the full text of every affected `gtt/context/` file, and an executable script:
...
**`gtt/proposals/` is the only directory under `gtt/` that an agent may write to as part of the governed change workflow.**
...
affected `gtt/context/` files, and generate the promotion script. It cannot
execute that script, edit `gtt/adr/` or `gtt/context/` directly, or treat a
...
wrapper. It lives in `gtt/proposals/`, names the proposal and ADR it belongs
...
second source of truth beside `gtt/context/`.

```text
Governed Context / L0  ->  ADR  ->  gtt/backlog.md  ->  Implementation
```
...
| Change | Path |
| --- | --- |
| New/removed Epic or Story, or a material scope/acceptance-criteria change | `gtt/CHANGE-REQUEST.md` → `gtt/proposals/` → Solution Designer decision |
...
from `gtt/backlog.md`. If Epics/Stories are defined elsewhere but missing from
...
│
gtt/                           # GTT governance — everything below is generated here, not at root
...
`README-GTT.md`/`.es.md` remain at the root because they are the human-facing entry points — the first thing anyone opening the project should be able to find, not something buried under `gtt/`.

`SOURCE-BRIEF.*` remains at the root for the same reason: it is the Solution Designer's own original design, in their own words, and should stay as discoverable as the READMEs. Everything else GTT owns — including `CHANGE-REQUEST.md`, now that it has one — lives under `gtt/`; see [Workspace hygiene](#workspace-hygiene).
...
| --- | --- | --- | --- |
| L0 | `gtt/context/` | Propose only | On demand, except `constraints.md` |
| L1 | `gtt/adr/` | Propose with review | On demand |
...
1. Copy the portable core — `AGENTS.md`, `README-GTT.md`, `README-GTT.es.md`, and `gtt/` in full (including `gtt/docs/`, `gtt/scripts/`, and the `INDEX.md`/`CHANGE-REQUEST.md`/`GTT-COMPLETION.md`/`backlog.md` already inside it) — into the project root, plus only the adapter matching your ADE: `.claude/` (including `.claude/CLAUDE.md`) for Claude Code, `.kiro/` for Kiro, `.copilot/copilot-instructions.md` for GitHub Copilot, or nothing extra for Codex. See [ADE adapters](#ade-adapters); do not copy the other adapters in "just in case."
2. Merge the kit's `.gitignore` into your existing `.gitignore`; do not overwrite an existing project file.
3. Run the `gtt-bootstrap` skill (for example, “bootstrap GTT” or “set up GTT”) instead of filling `gtt/context/` by hand — it performs step 1 above for you, deterministically.
4. If you prefer to author the context manually, start with `gtt/context/stack.md`. Leave a cell empty rather than guessing; an explicit unknown is preferable to an invented decision.
5. Adjust the `paths:` globs in `.claude/rules/` to match the host project's folder layout (Claude Code only).
6. Wire `gtt/scripts/gtt-check-stack.sh` and `gtt/scripts/gtt-check-backlog.sh` into CI against the default branch.
7. Run a session and inspect `/context`. Only the expected core rules and constraints should be loaded automatically.
8. Verify the guardrail: ask the agent to edit a protected context file such as `gtt/context/stack.md`. The write must be blocked by the applicable enforcement layer, not merely discouraged.
...
immediately after the upgrade when `gtt/context/` already contains real content. Until the freeze marker exists, that context may remain agent-writable.

Full file map: [`gtt/INDEX.md`](gtt/INDEX.md) · Migration guidance: [`gtt/docs/DOCS.md#migrating-from-gtt-v1`](gtt/docs/DOCS.md#migrating-from-gtt-v1)
...
- `gtt/context/` and `gtt/adr/`: applied through the governed process and not directly written by an agent once frozen.
- `gtt/backlog.md`: Epics/Stories change through `gtt/CHANGE-REQUEST.md` like an architectural decision; status and focus updates during routine implementation are direct edits.
- `gtt/CHANGE-REQUEST.md`: your entry point whenever a governed decision or the committed development line needs to change.

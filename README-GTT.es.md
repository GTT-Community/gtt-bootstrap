...
4. Comprueba si `gtt/context/` todavía contiene placeholders de plantilla.
...
Antes del freeze todavía no existe nada ratificado que proteger. Por eso el agente puede escribir `gtt/context/` durante este bootstrap inicial.

El freeze es una **acción humana**. Valida que el contexto ya no contenga placeholders y crea el marcador `gtt/.frozen`, que cambia el proyecto al régimen gobernado y protege las rutas gobernadas contra escrituras directas del agente.
...
Para los procedimientos detallados, consulta [gtt/INSTALLATION.es.md](gtt/INSTALLATION.es.md) y [gtt/USAGE.es.md](gtt/USAGE.es.md).
...
Consulta [gtt/INSTALLATION.es.md](gtt/INSTALLATION.es.md#instalación-manual).
...
completo, nunca más de un adaptador nativo. La distribución fuente de GTT
Bootstrap contiene todos los adaptadores porque es un catálogo; instalarlos
todos en un proyecto no es el flujo previsto.

**Núcleo portable:** `AGENTS.md`, `README-GTT.md`, `README-GTT.es.md`, `SOURCE-BRIEF.*` (si existió documento fuente) en la raíz del proyecto, más `gtt/` en sí — que contiene `INDEX.md`, `CHANGE-REQUEST.md`, `GTT-COMPLETION.md`, `backlog.md`, `INSTALLATION.md`/`.es.md`, `USAGE.md`/`.es.md`, `context/`, `adr/`, `proposals/`, `docs/`, `scripts/`.
...
>
> GTT requiere seleccionar el ADE que está ejecutando este bootstrap.
>
> Detectados:
> - Claude Code
> - Kiro
>
> Por favor confirma qué ADE está ejecutando actualmente el bootstrap de GTT.
...
**GTT mantiene sus artefactos de desarrollo gobernado dentro de `gtt/`.
...
La raíz del proyecto pertenece al proyecto del usuario; `gtt/` pertenece a
la gobernanza de GTT. Antes de este principio, los propios archivos de
gobernanza de GTT (`INDEX.md`, `CHANGE-REQUEST.md`, `GTT-COMPLETION.md`,
...
La prueba: *si soy un desarrollador usando GTT, ¿la raíz del proyecto se
ve como mi proyecto, mientras `gtt/` contiene claramente la maquinaria de
gobernanza de GTT?* La raíz debe contener solamente:
...
- Los puntos de entrada humanos de GTT (`README-GTT.md`, `README-GTT.es.md`, `SOURCE-BRIEF.*`).
- Los propios archivos del proyecto del usuario.

Esto es un cambio estructural, no un paso de limpieza: el proceso de
bootstrap genera estos artefactos directamente bajo `gtt/` — nunca los
...
| **`gtt/CHANGE-REQUEST.md`** | La puerta de entrada para solicitar un cambio | Cuando deba cambiar una decisión gobernada o la línea de desarrollo |

`SOURCE-BRIEF.*` permanece en la raíz del proyecto — el único artefacto de GTT que es puramente tuyo para encontrar rápido, nunca maquinaria de gobernanza. `CHANGE-REQUEST.md` vive bajo `gtt/` junto con lo que gobierna, pero sigue siendo el único punto de entrada que usás constantemente; nunca se vuelve más difícil de alcanzar solo porque se movió.

`gtt/context/stack.md` es el archivo que leerás con mayor frecuencia —el mapa de una pantalla de lo que es el sistema—, pero es un resultado y no un archivo que normalmente debas editar manualmente. Los cambios aprobados llegan mediante `gtt/CHANGE-REQUEST.md`.

---

## El mapa

`gtt/context/stack.md` responde **“¿qué es este sistema?”** sin abrir el código.
...
| 7 | Señales de drift | ¿Qué rutas fuera de `gtt/` cargan peso arquitectónico, y qué protegen? |
...
```text
gtt/CHANGE-REQUEST.md  ->  gtt/proposals/  ->  revisas y ejecutas un script  ->  gtt/adr/ + gtt/context/stack.md
...
Completa el bloque de solicitud en `gtt/CHANGE-REQUEST.md`: qué debe cambiar, por qué, qué lo desencadenó, alcance, impacto, riesgo y prioridad.
...
Tú apruebas la propuesta. El agente entonces prepara un **paquete de promoción** en `gtt/proposals/`: el borrador del ADR, el texto completo de cada archivo afectado bajo `gtt/context/`, y un script ejecutable:
...
**`gtt/proposals/` es el único directorio bajo `gtt/` donde el agente puede escribir como parte del flujo de cambios gobernados.**
...
preparar los archivos afectados bajo `gtt/context/`, y generar el script de
promoción. No puede ejecutar ese script, editar `gtt/adr/` o `gtt/context/`
...
conveniencia. Vive en `gtt/proposals/`, nombra en su cabecera la propuesta y
...
`gtt/backlog.md` es la línea de desarrollo: Epics, Stories, y el trabajo
...
nunca una segunda fuente de verdad junto a `gtt/context/`.

```text
Contexto Gobernado / L0  ->  ADR  ->  gtt/backlog.md  ->  Implementación
```
...
| Cambio | Vía |
| --- | --- |
| Epic o Story nueva/eliminada, o cambio material de alcance/criterios de aceptación | `gtt/CHANGE-REQUEST.md` → `gtt/proposals/` → decisión del Solution Designer |
...
aplicable desde `gtt/backlog.md`. Si hay Epics/Stories definidas en otro lugar
...
│
gtt/                           # gobernanza GTT — todo lo de abajo se genera aquí, no en la raíz
...
`README-GTT.md`/`.es.md` permanecen en la raíz porque son los puntos de entrada humanos — lo primero que cualquiera que abra el proyecto debería poder encontrar, no algo enterrado bajo `gtt/`.

`SOURCE-BRIEF.*` permanece en la raíz por la misma razón: es el diseño original del Solution Designer, en sus propias palabras, y debe ser tan descubrible como los READMEs. Todo lo demás que posee GTT — incluyendo `CHANGE-REQUEST.md`, ahora que tiene uno — vive bajo `gtt/`; ver [Higiene del workspace](#higiene-del-workspace).
...
| --- | --- | --- | --- |
| L0 | `gtt/context/` | Solo propuesta | Bajo demanda, excepto `constraints.md` |
| L1 | `gtt/adr/` | Propuesta con revisión | Bajo demanda |
...
1. Copia el núcleo portable — `AGENTS.md`, `README-GTT.md`, `README-GTT.es.md`, y `gtt/` completo (incluyendo `gtt/docs/`, `gtt/scripts/`, y `INDEX.md`/`CHANGE-REQUEST.md`/`GTT-COMPLETION.md`/`backlog.md` ya dentro de él) — en la raíz del proyecto, más únicamente el adaptador correspondiente a tu ADE: `.claude/` (incluyendo `.claude/CLAUDE.md`) para Claude Code, `.kiro/` para Kiro, `.copilot/copilot-instructions.md` para GitHub Copilot, o nada adicional para Codex. Ver [Adaptadores de ADE](#adaptadores-de-ade); no copies los demás adaptadores "por las dudas".
...
inmediatamente después de la actualización cuando `gtt/context/` ya contenga contenido real. Hasta que exista el marcador de freeze, ese contexto puede seguir siendo escribible por el agente.

Mapa completo de archivos: [`gtt/INDEX.md`](gtt/INDEX.md) · Migración: [`gtt/docs/DOCS.md#migrating-from-gtt-v1`](gtt/docs/DOCS.md#migrating-from-gtt-v1)
...
- `gtt/context/` y `gtt/adr/`: se aplican mediante el proceso gobernado y, una vez congelados, no deben ser escritos directamente por un agente.
- `gtt/backlog.md`: las Epics/Stories cambian vía `gtt/CHANGE-REQUEST.md` como una decisión arquitectónica; las actualizaciones de estado y foco durante implementación rutinaria son ediciones directas.

#!/usr/bin/env python3
"""GTT - drift detection (PostToolUse hook).
...
def already_warned(session_id: str, glob: str) -> bool:
    state_path = os.path.join(
        tempfile.gettempdir(), f"gtt-drift-{session_id}.json"
    )
    try:
    ...
    for glob, guards in load_signals():
        if fnmatch.fnmatch(target, f"*{glob}") or fnmatch.fnmatch(target, glob):
            if already_warned(session_id, glob):
                continue
            print(
                f"GTT DRIFT SIGNAL: {target} matches '{glob}', which guards "
                f"{guards}. Run the gtt-drift-response skill to assess "
